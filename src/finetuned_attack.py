...
from finetune.loops import training_loop, validation_loop
...
if __name__ == "__main__":
    ...
    # Initialize datasets
    data_path, metadata_csv, json_hypercategory = config["data_path"], config["metadata_csv"], config["esc_hypercategories"]
    train_folds, val_folds = config["train_folds"][:-1], config["train_folds"][-1:]
    train_dset = ESC50Dataset(data_path=data_path, metadata_csv=metadata_csv, hypercategory_mapping=json_hypercategory, folds = train_folds)
    val_dset = ESC50Dataset(data_path=data_path, metadata_csv=metadata_csv, hypercategory_mapping=json_hypercategory, folds = val_folds)
    test_dset = ESC50Dataset(data_path=data_path, metadata_csv=metadata_csv, hypercategory_mapping=json_hypercategory, folds= config["test_folds"])
    ...
    # Test Loop
    path_to_pt_file = os.path.join(PROJECT_PATH, "pretrained_models", config['pt_file'])
    validation_loop(model=model, path_to_pt_file=path_to_pt_file, test_dset=test_dset, batch_size=config["batch_size"],num_workers=config["num_workers"], device=config.get('device', 'cpu'))


    # Filter wav files
    filter_results = filter_predictions(model, test_dset, num_workers=config["num_workers"],device=config.get('device', 'cpu'))
    correct_wav_files = filter_results['filtered_wavs']

    logging.info(f"{filter_results['classification_report']}\n")
    logging.info(
        f"Correct: {len(correct_wav_files)} ({100*len(correct_wav_files)/len(test_dset):.2f} %)"
    )
