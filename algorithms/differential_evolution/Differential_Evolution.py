...
class DifferentialEvolutionAttacker:
    ...
    def __init__(self,
        ...
        if self.target_class and self.hypercategory_target:
            self.target_class_index = np.where(
                self.model.hypercategory_mapping == self.target_class)[0]
    ...
    def obj(self, noise, starting_class_index, starting_class_label):
        ...
        if self.SNR_norm is not None:
            clipped_audio = utils.add_normalized_noise(
                self.clean_audio, noise, self.SNR_norm)["adversary"]

        else:
            clipped_audio = np.clip(self.clean_audio + noise, -1.0, 1.0)

        inference_results = self.model.make_inference_with_waveform(
            clipped_audio)
        probs, predicted_class_idx, label = inference_results[
            "probs"], inference_results[
        ...
        if self.target_class:
            if (label == self.target_class):
                if self.verbosity:
                    print(
                        f'Attack Succeded from {starting_class_label} to {label}'
                    )
        ...
        fitness = objective_functions.get_fitness(self.objective_function,
                                                  **objective_function_kwargs)
        if self.verbosity:
    ...
    def check_bounds(self, mutated):
        mutated_bound = [
            clip(mutated[i], self.bounds[i, 0], self.bounds[i, 1])
    ...
    def crossover(self, mutated, target):
        ...
        trial = [
            mutated[i] if p[i] < self.de_hyperparameters["cr"] else target[i]
            for i in range(len(self.bounds))
        ]
        return np.array(trial)

    def optimization(self, starting_class_index: int,
        ...
        for _ in range(self.de_hyperparameters["pop_size"]):
            random_values = utils.generate_bounded_white_noise(
                self.clean_audio,
        ...
        all_fitness_results = [
            self.obj(ind, starting_class_index, starting_class_label)
            for ind in pop
        ]
        ...
        if (best_obj == float('-inf')):
            ...
            return {
                ...
                "snr_scale_factor":
                None
                if self.SNR_norm is None else adv_dict["snr_scale_factor"],
                "inferred_class":
        ...
        for i in range(1, self.de_hyperparameters["iter"] + 1):
            ...
            for j in range(self.de_hyperparameters["pop_size"]):
                ...
                candidates = [
                    candidate
                    for candidate in range(self.de_hyperparameters["pop_size"])
                    if candidate != j
                ]
                ...
                fitness_results_target = self.obj(pop[j], starting_class_index,
                                                  starting_class_label)
                obj_target = fitness_results_target["fitness"]

                fitness_results_trial = self.obj(trial, starting_class_index,
                                                 starting_class_label)
                ...
                if (obj_trial == float('-inf')):
                    ...
                    if self.SNR_norm is not None:
                        adv_dict = utils.add_normalized_noise(
                            self.clean_audio, obj_trial, self.SNR_norm)
                    ...
                    return {
                        ...
                        trial,
                        "adversary":
                        np.clip(self.clean_audio + trial, -1.0, 1.0)
                        if self.SNR_norm is None else adv_dict["adversary"],
                        "raw audio":
                        self.clean_audio,
                        "iterations":
                        ...
                        None if self.SNR_norm is None else adv_dict["max_amp"],
                        "snr_scale_factor":
                        None if self.SNR_norm is None else
                        adv_dict["snr_scale_factor"],
                        "inferred_class":
                ...
                if obj_trial < obj_target:
                    if self.verbosity:
                        ...
                        print(
                            f"Previous Obj: {obj_target}, Current Obj {obj_trial}"
                        )
                        print(
                            "Replace selected candidate with the trial vector")
                    pop[j] = trial
                    obj_all[j] = obj_trial
    ...
    def generate_adversarial_example(self, source_audio):
        ...
        inference_results = self.model.make_inference_with_waveform(
            self.clean_audio)
        starting_class_index, starting_class_label = inference_results[
            "predicted_class_idx"], inference_results["label"]
        ...
        inference_results = self.model.make_inference_with_waveform(
            results["adversary"])
        probs, final_confidence = inference_results[
            "probs"], inference_results["best_score"]
